...
这是一个面向国内封锁网络环境的一体化开发环境管理方案，通过分层 Docker 镜像构建、OSS/OBS 对象存储分发，实现目标机（Ubuntu x86_64 / openEuler x86_64）的完全离线容器化开发环境部署。

> **架构范围（当前阶段）**：仅实现 `linux/amd64`（x86_64）。arm64 支持作为**可选未来计划**，待 amd64 验证稳定后手动扩展，届时需为每个镜像单独构建并测试 arm64 变体。
...
| 层 | 目录 | 内容 |
|----|------|------|
| L0 | `base/` | Ubuntu 22.04 amd64 / openEuler 22.03 amd64，最小系统 + 国内镜像源 |
...
arm64 支持（Ubuntu arm64 / openEuler arm64）将在 amd64 验证稳定后按需手动构建。
届时需要为每个 Dockerfile 添加多架构 FROM 以及在 build-matrix.yml 中增加 `linux/arm64` 平台。
...
2. **使用国内镜像源**：
   - apt: 阿里云 mirrors.aliyun.com/ubuntu（Ubuntu）
   - yum: 华为云 mirrors.huaweicloud.com/openeuler（openEuler）
   - pip: mirrors.aliyun.com/pypi/simple/
   - Docker 加速: registry.cn-hangzhou.aliyuncs.com
3. **当前阶段仅构建 `linux/amd64`**，不需要多架构 platforms 配置
...
- 当前阶段固定使用 `linux/amd64`，从 GitHub 下载二进制时直接指定 x86_64（无需架构检测）
